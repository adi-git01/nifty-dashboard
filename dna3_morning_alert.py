"""
DNA3-V2.2 MORNING ALERT SYSTEM
================================
...
def build_telegram_message(data):
    ...
    regime_icon = {'BULL': '🟢', 'CAUTION': '🟡', 'BEAR': '🟠', 'CRISIS': '🔴'}.get(regime, '⚪')
    msg = f"<b>DNA3-V2.2 Morning Brief</b>\n"
    msg += f"<i>{date}</i> | {regime_icon} {regime} | Trail: {trail_pct*100:.0f}%\n\n"
    msg += f"Portfolio: Rs {equity:,.0f} ({total_ret:+.1f}%)\n"
    msg += f"Holdings: {count} | Cash: Rs {cash:,.0f} | CAGR: {cagr:+.1f}%\n"

    # Rebalance countdown
    last_reb = data.get('last_rebalance_date')
    rebal_freq = config.get('rebalance_days', 13)
    if last_reb and rebal_freq < 999:
        try:
            from datetime import timedelta
            last_reb_dt = datetime.strptime(last_reb, '%Y-%m-%d')
            days_since = (datetime.now() - last_reb_dt).days
            days_left = max(0, rebal_freq - days_since)
            if days_left <= 1:
                msg += f"\u26a1 <b>Rebalance DUE</b> (day {days_since}/{rebal_freq})\n"
            else:
                msg += f"\U0001f4c5 Rebalance in {days_left} days (day {days_since}/{rebal_freq})\n"
        except:
            pass

    # Danger zone stocks first
    danger_stocks = [h for h in holdings if h.get('Danger')]
    if danger_stocks:
        msg += f"\n🚨 <b>DANGER ZONE ({len(danger_stocks)})</b>\n"
        for h in danger_stocks:
            ticker = h.get('Ticker', '').replace('.NS', '')
            pnl = h.get('PnL%', 0)
            msg += f"  {ticker}: {h['Danger']}\n"
    ...
    for h in holdings[:15]:
        ...
        price = h.get('Price', 0)
        rs = h.get('RS_Score', 0)
        trail_stop = h.get('Trail_Stop', 0)
        icon = '🟢' if pnl >= 0 else '🔴'
        danger = ' ⚠️' if h.get('Danger') else ''
        msg += f"  {icon}{ticker}: Rs{price:.0f} ({pnl:+.1f}%) RS:{rs} Stop:{trail_stop:.0f}{danger}\n"
    ...
    msg += f"\n<i>OptComp-V22 | Alpha Trend Dashboard</i>"
    return msg
...
def build_email_html(data):
    ...
    if danger_stocks:
        danger_html = f'''
            <div style="background: rgba(255,23,68,0.15); border: 1px solid #FF1744; border-radius: 8px; padding: 12px; margin-bottom: 20px;">
                <strong style="color: #FF1744;">🚨 DANGER ZONE ({len(danger_stocks)} stocks near exit)</strong><br>
        '''
        for h in danger_stocks:
            ticker = h.get('Ticker', '').replace('.NS', '')
            danger_html += f'<span style="color: #FF5252;">{ticker}: {h["Danger"]}</span><br>'
        danger_html += '</div>'
    ...
    <head>
        <style>
            ...
            .regime {{ display: inline-block; background: {regime_color}22; border: 1px solid {regime_color}; color: {regime_color}; padding: 4px 12px; border-radius: 20px; font-size: 13px; font-weight: bold; }}
            ...
            .danger {{ color: #FF5252; font-size: 11px; }}
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>DNA3-V2.2 Morning Brief</h1>
                <p>{date} | <span class="regime">{regime} | Trail: {trail_pct*100:.0f}%</span></p>
            </div>
            ...
            <div style="background: rgba(102,126,234,0.1); border: 1px solid #667eea; border-radius: 8px; padding: 10px; margin-bottom: 20px; text-align: center; font-size: 13px;">
    """
    # Rebalance countdown for email
    last_reb = data.get('last_rebalance_date')
    rebal_freq = config.get('rebalance_days', 13)
    if last_reb and rebal_freq < 999:
        try:
            ...
            days_left = max(0, rebal_freq - days_since)
            if days_left <= 1:
                html += f'\u26a1 <strong>Rebalance DUE</strong> (day {days_since}/{rebal_freq})'
            else:
                html += f'\U0001f4c5 Rebalance in <strong>{days_left} days</strong> (day {days_since}/{rebal_freq})'
        except:
            html += 'Rebalance info unavailable'
    else:
        html += 'Rebalance: HOLD (crisis mode)' if rebal_freq >= 999 else ''
    html += """
            </div>
            
            <table>
                <tr><th>Stock</th><th>Price</th><th>P&L</th><th>RS</th><th>Trail Stop</th><th>Risk</th></tr>
    """
    ...
    for h in holdings[:15]:
        ...
        danger_td = f'<span class="danger">{danger}</span>' if danger else '✅'
        html += f"""
                <tr>
