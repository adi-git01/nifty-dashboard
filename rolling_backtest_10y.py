"""
10-Year Rolling Window Backtester
=================================
Validates strategy performance across 10 years (2016-2026).
Covers: 2016 Demonetization, 2017 Bull Run, 2018 Midcap Crash, 
2020 Covid Crash, 2021 Post-Covid Rally, 2022 Inflation Bear, 2023-24 Bull Run.

Methodology:
1. Window: 6 Months (126 trading days)
2. Step: 1 month (20 trading days)
3. Period: Last 10 Years
4. Goal: Identify regimes where 20% return is possible.
"""
...
class RollingBacktest10Y:
    ...
    def simulate_window(self, start_date, end_date):
        ...
        if portfolio:
            ...
            best_sector = max(set(sectors), key=sectors.count)
    ...
    def run(self):
        ...
        df.to_csv('analysis_2026/rolling_backtest_10y.csv', index=False)
        
        print("\n" + "="*60)
        print("ROLLING BACKTEST COMPLETE (10 Years)")
        print("="*60)
        print(f"Total Windows: {len(df)}")
        print(f"Avg Portfolio Return: {df['portfolio_return'].mean()*100:.2f}%")
        print(f"Avg Nifty Return:     {df['nifty_return'].mean()*100:.2f}%")
        print(f"Win Rate (>0%):       {(df['portfolio_return']>0).mean()*100:.1f}%")
        print(f"20% Hit Rate:         {(df['portfolio_return']>=TARGET_RETURN).mean()*100:.1f}%")
