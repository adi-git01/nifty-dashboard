"""
EARLY MOMENTUM STRATEGY: 3-YEAR BACKTEST
==========================================
Catches stocks EARLIER in their momentum phase - when trends are emerging,
not when they're already fully "hot".

Key Differences from DNA3-V2:
1. Trend Score: 65-85 (emerging) instead of 90+ (established)
2. RS: 0-30% (moderate) instead of 2%+ (any positive)
3. Price just crossed above MA50 recently (within 10 days)
4. Volatility: 25-50% (moderate, not extreme)
"""
...
class EarlyMomentumBacktest:
    ...
    def fetch_data(self):
        print(f"[EARLY MOMENTUM] Fetching data for {YEARS}-year backtest...")
    ...
    def calculate_trend_score(self, window):
        """Calculate trend score based on MA relationships."""
        if len(window) < 200:
            return 0
    ...
    def passes_early_momentum_filter(self, ticker, date):
        """EARLY MOMENTUM Entry Filters - Catch trends early, not late."""
        ...
        # === TREND SCORE: Target 65-85 (Emerging, not hot) ===
        trend_score = self.calculate_trend_score(window)
        if trend_score < 65 or trend_score > 85:
            return False, {}
        
        # === RS: Moderate (0% to 30%) - Not too hot ===
        ...
        if rs < 0 or rs > 30:  # Must be positive but not extreme
            return False, {}
        
        # === VOLATILITY: Moderate (25-50%) ===
        ...
        if volatility < 25 or volatility > 50:
            return False, {}
        
        # === KEY: RECENT MA50 CROSSOVER (within last 10 days) ===
        ma50_series = window['Close'].rolling(50).mean()
        
        # Check if price crossed above MA50 recently
        recent_crosses = 0
        for i in range(-10, 0):
            if len(window) > abs(i) + 1:
                ...
                # Crossed from below to above MA50
                if prev_price < prev_ma50 and curr_price > curr_ma50:
                    recent_crosses += 1
        
        # Must have crossed above MA50 recently OR be just above it
        ma50 = ma50_series.iloc[-1]
        above_ma50_pct = (price - ma50) / ma50 * 100
        
        if recent_crosses == 0 and above_ma50_pct > 5:
            # Too far above MA50 without recent crossover = already hot
            return False, {}
    ...
    def select_with_sector_cap(self, candidates):
        """Apply sector cap when selecting candidates."""
        # Sort by RS (prefer moderate RS, penalize extreme)
        candidates.sort(key=lambda x: -x['rs'])  # Still pick higher RS within our 0-30% band
    ...
    def run(self):
        ...
        print(f"\n[EARLY MOMENTUM] Running {YEARS}-Year Backtest...")
        ...
        for date in dates:
            ...
            for t, pos in self.positions.items():
                ...
                if price < pos['stop']:
                    ...
                    self.trade_log.append({
                        ...
                        'entry_trend': pos.get('entry_trend', 0)
                    })
                    to_exit.append(t)
            ...
            if len(self.positions) < MAX_POSITIONS:
                ...
                for ticker in self.data_cache.keys():
                    ...
                    passes, metrics = self.passes_early_momentum_filter(ticker, date)
                    if passes:
                        candidates.append({
                            'ticker': ticker, 
        ...
        print("\n" + "="*70)
        print(f"[EARLY MOMENTUM] RESULTS ({YEARS} Years)")
        print("="*70)
        print(f"\n{'Metric':<25} {'Early Momentum':<20} {'Nifty 50':<20}")
        print("-"*70)
        ...
        if self.trade_log:
            ...
            # Entry trend score analysis
            avg_entry_trend = np.mean([t.get('entry_trend', 0) for t in self.trade_log])
            print(f"  Avg Entry Trend Score: {avg_entry_trend:.1f}")
        ...
        # Comparison header
        print("\n" + "="*70)
        print("COMPARISON: Early Momentum vs DNA3-V2.1 (run both to compare)")
        print("="*70)
        print("DNA3-V2.1 (Hot Momentum):  Trend 90+,  RS 2%+,    Vol 30%+")
        print("Early Momentum:            Trend 65-85, RS 0-30%, Vol 25-50%")
        
        # Save results
        df.to_csv('analysis_2026/early_momentum_backtest_3y.csv', index=False)
        trades_df = pd.DataFrame(self.trade_log)
        trades_df.to_csv('analysis_2026/early_momentum_trades_3y.csv', index=False)
        print(f"\n[SAVED] analysis_2026/early_momentum_backtest_3y.csv")
        print(f"[SAVED] analysis_2026/early_momentum_trades_3y.csv")
        
        return total_ret, n_ret

if __name__ == "__main__":
    bt = EarlyMomentumBacktest()
    bt.run()
