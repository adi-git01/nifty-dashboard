"""
QVR-V3.1 FUSION STRATEGY BACKTEST
===================================
Concept:
- Entry: QVR Logic (Quality >= 6, Value >= 6, Trend 40-65)
  - Buy high-quality, attractively-valued companies that are just 
    starting to recover from a bottom.
- Exit: DNA3-V3.1 Logic
  - Let winners run. NO exit at Trend 75. 
  - Exit ONLY on 12% trailing stop from peak or -20% hard stop.
- Sizing: Regime-adaptive (same as V3.1)
...
# ============================================================
# SCORE PROXIES (matching prior script)
# ============================================================
...
class QVRFusionEngine:
    ...
    def check_exits(self, dc, d):
        ...
        for t, pos in self.positions.items():
            ...
            # FUSION EXIT: Only Trailing Stop and Hard Stop
            reason = None
            if p < pos['peak'] * 0.88: reason = 'Trail'
            if (p - pos['entry']) / pos['entry'] < -0.20: reason = 'HardStop'
            
            if reason:
                self.capital += pos['shares'] * p * (1 - COST_BPS/10000)
                self.trade_log.append({
                    'Ticker': t, 'PnL%': round((p - pos['entry'])/pos['entry']*100, 2),
...
def run():
    ...
    print(f"\n{'=' * 100}")
    print(f"QVR-FUSION vs DNA3-V3.1 vs NIFTY ({YEARS}Y BACKTEST)")
    print(f"{'=' * 100}")
    
    strategies = {'QVR-Fusion': QVRFusionEngine(), 'DNA3-V3.1': V31Engine()}
    results, trade_stats = {}, {}
    ...
    print(f"\n  {'Metric':<18} {'QVR-Fusion':>12} {'V3.1 (Mom)':>12} {'Nifty':>12}")
    print(f"  {'-'*58}")
    for k in ['CAGR%', 'MaxDD%', 'Sharpe', 'Trades', 'WinRate%', 'AvgWin%', 'AvgLoss%', 'MaxWin%', 'AvgHold', 'Expectancy%']:
        f = results['QVR-Fusion'].get(k, '-')
        v = results['DNA3-V3.1'].get(k, '-')
        n = round(n_cagr, 2) if k == 'CAGR%' else '-'
        print(f"  {k:<18} {f:>12} {v:>12} {n:>12}")
    
    # QVR-Fusion Best Trades
    print(f"\n{'_' * 100}")
    print(f"  QVR-FUSION BIGGEST WINNERS")
    print(f"{'_' * 100}")
    top_trades = sorted(trade_stats['QVR-Fusion'], key=lambda x: -x['PnL%'])[:5]
    for i, t in enumerate(top_trades, 1):
        print(f"  {i}. {t['Ticker']:<15} {t['PnL%']:>+7.1f}% (Held {t['Hold']:.0f} days)")
