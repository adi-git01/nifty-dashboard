"""
DNA-3 V2: 10-YEAR BACKTEST + CURRENT PICKS + EXPANDED UNIVERSE
===============================================================
"""
...
# EXPANDED UNIVERSE (Added Pharma, Banking, Energy, Consumer)
UNIVERSE = {
    'Industrials': ['LT.NS', 'SIEMENS.NS', 'ABB.NS', 'HAVELLS.NS', 'CUMMINSIND.NS', 'POLYCAB.NS', 'BEL.NS', 'HAL.NS', 'THERMAX.NS', 'CROMPTON.NS'],
    ...
    'Auto': ['MARUTI.NS', 'M&M.NS', 'BAJAJ-AUTO.NS', 'HEROMOTOCO.NS', 'EICHERMOT.NS', 'TVSMOTOR.NS', 'ASHOKLEY.NS', 'MOTHERSON.NS', 'BHARATFORG.NS'],
    ...
    'Consumer': ['HINDUNILVR.NS', 'ITC.NS', 'TITAN.NS', 'TRENT.NS', 'DABUR.NS', 'BRITANNIA.NS', 'NESTLEIND.NS']
}

class DNA3V2Extended:
    ...
    def fetch_data(self):
        print(f"[DNA-3 V2 Extended] Fetching {self.years}+ years of data...")
    ...
    def run_backtest(self):
        ...
        print(f"[DNA-3 V2] Running {self.years}-Year Backtest...")
        ...
        print("\n" + "="*60)
        print(f"[DNA-3 V2 EXPANDED] RESULTS ({self.years} Years)")
        print("="*60)
    ...
    def get_current_picks(self):
        """Get stocks passing DNA-3 V2 filters TODAY."""
        print("\n" + "="*60)
        print("[DNA-3 V2] CURRENT STOCK PICKS (Today)")
        print("="*60)
        ...
        for sector, tickers in UNIVERSE.items():
            for ticker in tickers:
                passes, metrics = self.passes_dna_filter(ticker, today)
                if passes:
                    picks.append({
                        'Ticker': ticker.replace('.NS', ''),
                        'Sector': sector,
                        'RS vs Nifty': f"+{metrics['rs_3m']:.1f}%",
                        'Volatility': f"{metrics['volatility']:.1f}%",
                        'Price': f"Rs.{metrics['price']:.2f}"
                    })
        
        if picks:
            # Sort by RS
            picks.sort(key=lambda x: float(x['RS vs Nifty'].replace('+','').replace('%','')), reverse=True)
            df = pd.DataFrame(picks)
            print(df.to_string(index=False))
            df.to_csv('analysis_2026/dna3_current_picks.csv', index=False)
            print(f"\nTotal Picks: {len(picks)}")
        else:
            print("No stocks currently passing DNA-3 V2 filters.")
        
        return picks

if __name__ == "__main__":
    # 1. Run 10-Year Backtest
    strat = DNA3V2Extended(years=10)
    strat.run_backtest()
    
    # 2. Get Current Picks
    strat.get_current_picks()
